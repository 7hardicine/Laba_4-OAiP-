...
void main()
{
	...
	while (abs((xk - xn) / dx) >= i) // Цикл для проверки всего промежутка
	{
		if ((x == 2) || (x == -1) || (x == 0)) // Проверкка каждого значения промежутка на область определения
		{
			...
			dx = 0; 
			while ((scanf("%lf", &dx) < 1) || (xn > xk && dx > 0) || (xn < xk && dx < 0)) // Повторный ввод шага + проверка на выполнение программы
			{
				puts("Вы ввели что-то не так, введите значение заново");
				while (getchar() != '\n');
			}
	...
	if (dx == 0) // Вычисление Y если шаг равен 0
	{
		y = exp(-x) * (a + b * x) / (log(abs(x + 1)) * log(abs(x + 1)));
		printf("%2d | x = %5.1lf | y = %5.5lf\n", i + 1, x, y);
	}
	else // Считаем Y если шаг != 0
	{
